...
public class Main {
    ...
    private static Equation parseEquation(final String equation) {
        ...
        final long[] operands = Arrays.stream(parts[1].split(" ")).mapToLong(Long::parseLong).toArray();
        return new Equation(target, operands);
    }

    private static Equation[] parseInput(final String input) {
        return Arrays.stream(input.split(System.lineSeparator())).map(Main::parseEquation).toArray(Equation[]::new);
    }
    ...
    private static boolean isPossible2(final Equation equation, final long accumulated, final int current) {
        ...
        return Main.isPossible2(equation, accumulated + operands[current], current + 1) ||
                ...
                Main.isPossible2(equation, Long.parseLong(Long.toString(accumulated) + Long.toString(operands[current])), current + 1);
    }
